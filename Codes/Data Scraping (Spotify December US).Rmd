...
Top 200 hits on Spotify from the USA, Jun 1st to September 30th.

Constant -----------------------------------/Variable
https://spotifycharts.com/regional/us/daily/2018/02/01
https://spotifycharts.com/regional/us/daily/2018/02/28


1. Fix the Constant, this is for US only.
```{r}
...
```{r}
timevalues <- seq(as.Date("2017/12/01"), as.Date("2018/11/30"), by = "day")
glimpse(timevalues)
str(timevalues)
```
...
Create a flag for US
```{r}
max_row<-NROW(spotify)
list1<- 1:max_row
Country <- rep("USA",length(list1))
...
```{r}
# Write CSV in R
write.csv(spotify2, file = "spotify_USA_12M.csv")
# Saving object to a file.
saveRDS(spotify2, file = "spotify_USA_12M.rds")
```

Compile all together:
```{r}
set1 <- readRDS("spotify_USA_0M.rds")
set2 <- readRDS("spotify_USA_2_12M.rds")
head(usa)
```
